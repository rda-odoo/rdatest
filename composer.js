...
var BasicComposer = Widget.extend({
    ...
    start: function () {
        ...
        this.on("submit:form.o_form_binary_form",this, this.on_file_upload);
    ...
    on_attachment_change: function(event) {
        ...
        // console.log('<<<<<<<<<<ttttttttt',$target["0"].files);
        if ($target.val() !== '') {
            ...
            console.log('>>>>>>>>>>>>dataaaa',this.$('form.o_form_binary_form')["0"][2].files);
            // submit filename
            var a = this;
           this.$('form.o_form_binary_form').submit(function (e) {
                console.log('dataaaaaaaaaaa',attachments);
    
            // return session.rpc('/web/binary/upload_attachment', {
            // model: "mail.compose.message",
            // ufile: a.$('form.o_form_binary_form')["0"][2].files,
            // callback:a.fileupload_id,
            // id:0,
            // });
        ...
        data.append('callback', a.fileupload_id);
        data.append('model', 'mail.compose.message');
        data.append('id', '0');
        var datarray = [];
        for (var i = 0; i < a.$('form.o_form_binary_form')["0"][2].files.length; i++) {
        datarray.push(a.$('form.o_form_binary_form')["0"][2].files[i]);
        }
        console.log("datarray",datarray);
        data.append('ufile',datarray);

        
          // var xhr = new XMLHttpRequest();
          //   var url = encodeURI('/web/binary/upload_attachment');
          //   xhr.open('POST', url);
          //   xhr.send(data);

        console.log("datarrayssssssssssss",data);
        ...
        $.ajax({
            ...
            _.each($target["0"].files, function(a, index) {
            attachments.push({
                'id': index,
                'name': a.name,
                'filename': a.name,
             ...
             console.log("arrrrrrrrrrrrrrrta",attachments)
            this.set('attachment_ids', attachments);
        }
    },
    on_file_upload: function(event){
     var attachment_ids = [];
     console.log("MMMMMMMM",this.get('attachment_ids'));
    },
    on_attachment_loaded: function(event, result) {
        console.log('ttttttttttttttt',event);
        console.log('rrrrrrrrrrrrrrrrrreeeeee',result)
        var attachment_ids = [];
        console.log('aaaaaaaaaa>>>>>>>>>>>>>',this.get('attachment_ids'));
        if (result.error || !result.id ) {
            this.do_warn(result.error);
            attachment_ids = _.filter(this.get('attachment_ids'), function (val) { return !val.upload; });
        } else {
            _.each(this.get('attachment_ids'), function(a,index) {
                if (a.upload) {
                    // console.log('aaaaaaaaaa',result.id);
                    attachment_ids.push({
                        'id': result.id,
